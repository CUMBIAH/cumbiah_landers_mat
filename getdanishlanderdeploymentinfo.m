...
%GETDANISHLANDERDEPLOYMENTINFO Deployment times and device IDs for one CUMBIAH
%Denmark lander.
%
%   [STARTTIME, ENDTIME, SERIALNUMBER, HYDROPHONEIDS] =
%   GETDANISHLANDERDEPLOYMENTINFO(LANDERNUMBER, DEPLOYMENTNUMBER) returns
%   the times and the instruments for one deployment of one Danish seabed
%   lander. It is normally called through GETLANDERDEPLOYMENTINFO with
%   COUNTRY 'Denmark'.
%
...
%   GETDANISHLANDERPATHS, so a value TIME satisfies STARTTIME <= TIME < ENDTIME
%   exactly when GETDANISHLANDERPATHS resolves to this deployment.
%
...
%   the deployment - which is why GETLANDERPATHS needs a time as well.
%
...
%
%   Example:
%       [t0, t1, sn] = getdanishlanderdeploymentinfo(1, 2);   % lander 1, Feb 25
%       data = getlanderpaths(sn, t0);                      % data.sqlitedB, ...
%
%   See also GETLANDERDEPLOYMENTINFO, GETDANISHLANDERPATHS, CUMBIAHLANDERTABLE.

narginchk(2, 2);
if ~isnumeric(landernumber) || ~isscalar(landernumber)
    error('getdanishlanderdeploymentinfo:badLander', 'LANDERNUMBER must be a scalar, e.g. 1.');
end
if ~isnumeric(deploymentnumber) || ~isscalar(deploymentnumber)
    error('getdanishlanderdeploymentinfo:badDeployment', ...
