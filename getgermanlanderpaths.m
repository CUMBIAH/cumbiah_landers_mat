function data = getgermanlanderpaths(serialnumber, time) %#ok<INUSD>
%GETGERMANLANDERPATHS Provides links to the CUMBIAH Germany lander data.
%
%   DATA = GETGERMANLANDERPATHS(SERIALNUMBER,TIME) - NOT IMPLEMENTED YET. It
%   always returns CUMBIAHDATADEFAULTS(), i.e. "not a German SoundTrap", so
...
%   a datetime), find the German deployment that SoundTrap was in at that
%   time and return one struct DATA with every field of CUMBIAHDATADEFAULTS
%   filled in:
%
%       landernumber       - lander number, as the German team numbers them
%       deployment         - deployment number within Germany
...
%   positive. Paths should be rooted on GETLANDERSUPERFOLDER('Germany') so
%   they follow the Dropbox folder between machines.
%
%   If no German deployment matches SERIALNUMBER at TIME, return
...
%   from a settings spreadsheet in the Germany folder, as CUMBIAHLANDERTABLE
%   does for Denmark, rather than written into the code.
%
%   See also GETLANDERPATHS, GETDANISHLANDERPATHS, GETGERMANLANDERDEPLOYMENTINFO,
%   CUMBIAHDATADEFAULTS.

% not implemented - no German SoundTrap is known yet
